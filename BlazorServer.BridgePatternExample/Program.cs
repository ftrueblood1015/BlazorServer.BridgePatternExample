using BlazorServer.BridgePatternExample.Data;
...
builder.Services.AddDbContext<BridgePatternContext>(opt => opt.UseSqlite(builder.Configuration.GetConnectionString("BridgePattern")));
...
async Task EnsureDbIsMigrated(IServiceProvider services)
{
    ...
    using var ctx = scope.ServiceProvider.GetService<BridgePatternContext>();
    if (ctx != null)
    {
